 ...
 *
 * Distinct from the Update sidebar above: the picker lists the characters the user may view
 * (GetAffiliatedCharactersController, fetched via the native-fetch getJson helper + Wayfinder),
 * lets you tick a subset, and on close navigates the page with `character_ids[]` so the list
 * filters to the selection. This asserts the whole round-trip works without Ziggy/axios: the
 * picker lists both affiliated characters, selecting one writes it to the URL query the page
 * controller reads, and the "has selection" indicator lights up after the reload.
 *
