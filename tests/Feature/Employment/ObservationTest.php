...
it('lists only corporations that have SSO scopes configured', function () {
    $configured = $this->test_character->corporation;
...
it('exposes a corporation\'s members with compliance, activity and employment status as a deferred prop', function () {
    $corp = $this->test_character->corporation;
    ...
    Employment::factory()->create([
        ...
        'subject_id' => $this->test_user->getKey(),
    ]);
    ...
    $this->actingAs($this->test_user)
        ...
        ->assertOk()
        ->assertJsonPath("props.members.{$corp->corporation_id}.0.id", $this->test_user->getKey())
...
it('inspects a member with the shared review tabs', function () {
    ...
    $this->actingAs($this->test_user)
        ->get(route('employment.observe.member', [$corp->corporation_id, $this->test_user->getKey()]))
        ->assertOk()
        ->assertInertia(fn (Assert $page) => $page
