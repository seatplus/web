...
test('one can update the locale', function () {
    test()->actingAs(test()->test_user);

    $response = test()->post(route('user.settings.locale'), [
        'locale' => 'de',
    ]);
...
test('one can not update to an unsupported locale', function () {
    test()->actingAs(test()->test_user);
    ...
    expect(test()->test_user->refresh()->locale)->toBeNull();
});

test('the application locale follows the user preference', function () {
...
test('the missing translation keys command runs', function () {
    test()->artisan('seatplus:i18n:missing-keys')
        ->assertSuccessful();
});
