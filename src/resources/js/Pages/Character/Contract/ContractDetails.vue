    ...
    <Layout page="Contract Details">
        ...
        <div class="grid gap-4">
            <CardWithHeader v-if="requested_items.length > 0">
                ...
                <wide-lists class="max-h-96 overflow-y-auto">
                    <template v-slot:elements>
                        <wide-list-element v-for="(item, index) in requested_items" :key="item.record_id">
                            <template v-slot:avatar>
                        <span class="inline-block relative">
                            <EveImage :tailwind_class="'h-12 w-12 rounded-full text-white shadow-solid bg-white'" :object="item.type" :size="128" :bpo="item.raw_quantity === -1" />
                            <span v-if="item.quantity > 1" class="absolute bottom-0 right-0 inline-flex items-center justify-center h-3 w-3 rounded-full text-white shadow-solid bg-gray-400">
                            ...
                            <template slot="upper_left">{{item.name}}</template>
                            ...
                            <template slot="upper_right">
                                {{item.type.group.name}}
                                <span v-if="isBPO(item)" class="text-info">
                            (Blueprint Original)
                        </span>
                                <span v-else-if="isBlueprint(item)" class="text-info">
                            (Blueprint Copy)
                        </span>
                                <span v-else-if="!item.is_singleton" class="text-info">
                            (packaged)
                        </span>
                            </template>
            ...
            <CardWithHeader v-if="included_items.length > 0">
                ...
                <wide-lists class="max-h-96 overflow-y-auto">
                    <template v-slot:elements>
                        <wide-list-element v-for="(item, index) in included_items" :key="item.record_id">
                            <template v-slot:avatar>
                        <span class="inline-block relative">
                            <EveImage :tailwind_class="'h-12 w-12 rounded-full text-white shadow-solid bg-white'" :object="item.type" :size="128" :bpo="item.raw_quantity === -1" />
                            <span v-if="item.quantity > 1" class="absolute bottom-0 right-0 inline-flex items-center justify-center h-3 w-3 rounded-full text-white shadow-solid bg-gray-400">
