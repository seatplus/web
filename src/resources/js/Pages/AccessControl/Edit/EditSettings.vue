              ...
              <div :class="[index%2 === 0 ? 'bg-white' : 'bg-gray-50', 'w-full sm:w-1/2 py-4 px-6']">
                <span class="relative z-0 inline-flex  rounded-md">
                  ...
                  <button
                      ...
                      @click="addAffiliation('allowedValue', entity)"
                  >
                    Allowed
                  </button>
                  <button
                      ...
                      @click="addAffiliation('inverseValue', entity)"
                  >
                    Inverse
                  </button>
                  <button
                      ...
                      @click="addAffiliation('forbiddenValue', entity)"
                  >
                    Forbidden
                  </button>
                </span>
    ...
    <div class="mb-3 grid gap-5 max-w-lg mx-auto lg:grid-cols-3 lg:max-w-none">
      <div class="flex flex-col rounded-lg shadow-lg overflow-hidden">
        <div class="flex-1 bg-white flex flex-col justify-between">
          ...
          <AffiliationList v-model="allowedValue" />
        </div>
      </div>
      <div class="flex flex-col rounded-lg shadow-lg overflow-hidden">
        <div class="flex-1 bg-white flex flex-col justify-between">
          ...
          <AffiliationList v-model="inverseValue" />
        </div>
      </div>
      <div class="flex flex-col rounded-lg shadow-lg overflow-hidden">
        <div class="flex-1 bg-white flex flex-col justify-between">
          ...
          <AffiliationList v-model="forbiddenValue" />
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  emits: ['update:allowed', 'update:inverse', 'update:forbidden'],
  data() {
    return {
      allowedValue: this.allowed,
      inverseValue: this.inverse,
      forbiddenValue: this.forbidden,
      entities: [],
      search: null
    }
  },
  computed: {
    filteredEntities() {
      ...
      let entities = _.filter(this.entities, (entity) => !_.includes(this.assignedEntityIds, entity.id))
  ...
  watch: {
    ...
    /*allowedValue(newVal) {
          this.$emit('update:allowed', newVal)
      },
    inverseValue(newVal) {
      this.$emit('update:inverse', newVal)
      },
    forbiddenValue(newVal) {
      this.$emit('update:forbidden', newVal)
      },*/
  },
  mounted() {
  ...
  methods: {
    addAffiliation(type, entity) {
      ...
      this.$emit(`update:${type.slice(0,-5)}`, toRaw(this[type]))
    },
