<template>
    <div :class="['shadow-lg rounded-lg pointer-events-auto border', colorSchema.background]">
        <div class="rounded-lg shadow-xs overflow-hidden">
            <div class="p-4">
                <div class="flex items-start">
                    <div class="flex-shrink-0">
                        <svg v-if="colorSchema.iconType === 'outline'" :class="['h-6 w-6', colorSchema.icon]" stroke="currentColor" fill="none" viewBox="0 0 24 24" v-html="this.icon"></svg>
                        <svg v-if="colorSchema.iconType === 'solid'" :class="['h-6 w-6', colorSchema.icon]" fill="currentColor" viewBox="0 0 20 20" v-html="this.icon"></svg>
                    </div>
                    <div class="ml-3 w-0 flex-1 pt-0.5">
                        <p :class="['text-sm leading-5 font-medium', colorSchema.title]">
                            {{ this.title }}
                        </p>
                        <p :class="['mt-1 text-sm leading-5', colorSchema.text]">
                            {{ this.text }}
                        </p>
                    ...
                    <div class="ml-4 flex-shrink-0 flex">
                        <button @click="$emit('remove', id)" :class="['inline-flex focus:outline-none transition ease-in-out duration-150', colorSchema.button]">
    ...
    export default {
        ...
        data() {
            return {
                ...
                defaultColorSchema: {
                    ...
                    title: 'text-gray-900',
                    text: 'text-gray-500',
                    button: 'text-gray-400 focus:text-gray-500'
                },
                colorSchemas: {
                    info: {
                        color: 'blue',
                    ...
                    error: {
                        color: 'red',
                        svg: ' <path fill="currentColor" fill-rule="evenodd" clip-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" />',
                    },
                    success: {
                        color: 'green',
        ...
        computed: {
            ...
            colorSchema() {
                ...
                return  {
                    background: `bg-${typedSchema.color}-50`,
                    icon: `text-${typedSchema.color}-400`,
                    iconType: `solid`,
                    svg: `${typedSchema.svg}`,
                    title: `text-${typedSchema.color}-800`,
                    text: `text-${typedSchema.color}-700`,
