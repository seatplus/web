  ...
  <nav class="md:flex-1 px-2 py-4 md:bg-gray-800">
    <div v-for="(category, name, index) in sidebarEntries" :key="name">
      ...
      <Link
        ...
        :class="[{'mt-1': index > 0, 'text-white bg-gray-900': isActive(entry.route), 'text-gray-300 hover:text-white hover:bg-gray-700 focus:text-white': !isActive(entry.route)},'group flex items-center px-2 py-2 md:text-sm text-base leading-5 md:leading-6 font-medium rounded-md focus:outline-hidden focus:bg-gray-700 transition ease-in-out duration-150']"
      >
        <svg
          v-if="entry.content"
